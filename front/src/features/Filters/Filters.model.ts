...
export type FiltersModelT = {
  ...
  ) => Promise<TransactionsUniqueFilterValues>;
  getCardsFilters: (institutionID: number) => Promise<CardsUniqueFilterValues>;
};

const EMPTY_TRANSACTION_FILTERS: TransactionsUniqueFilterValues = {
  cardIDs: [],
  cardAccount: [],
  currency: [],
};
const EMPTY_CARDS_FILTERS: CardsUniqueFilterValues = {
  cardIDs: [],
  cardAccount: [],
  currency: [],
  status: [],
...
class FiltersModel implements FiltersModelT {
  ...
  @observable transactionsFilters: TransactionsUniqueFilterValues =
  ...
  private getFilters = async <
    T = TransactionsUniqueFilterValues | CardsUniqueFilterValues
  >(
    institutionID: number,
    filterFor: 'transactions' | 'cards'
  ): Promise<UniqueFilterResponse<T> | undefined> => {
  ...
  getTransactionsFilters = async (
    institutionID: number
  ): Promise<TransactionsUniqueFilterValues> => {
    this.transactionsFilters = EMPTY_TRANSACTION_FILTERS;
    const resp = await this.getFilters<TransactionsUniqueFilterValues>(
      institutionID,
      'transactions'
    );
  ...
  getCardsFilters = async (
    institutionID: number
  ): Promise<CardsUniqueFilterValues> => {
    this.cardsFilters = EMPTY_CARDS_FILTERS;
    const resp = await this.getFilters<CardsUniqueFilterValues>(
      institutionID,
      'cards'
    );
