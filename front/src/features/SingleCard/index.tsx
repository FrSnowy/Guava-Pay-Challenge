...
export const CardInformation: React.FC<Omit<Card, 'cardAccountMeta'> & { withLink?: string }> = ({ withLink, ...card }) => {
    ...
    <div className={style.container}>
      ...
      <div className={style.meta}>
        <Grid columns={2}>
          ...
          <Grid.Column width={9}>
            <div>Sorry, but I really don't know what else we can show for the card.</div>
            <div>Let's just imagine that some important information located here too</div>
          </Grid.Column>
        </Grid>
      </div>
    </div>
...
const SingleCardPage = observer(() => {
  ...
  const { cardID: qsCardID, transactionID: qsTransactionID } = useParams<{ cardID: string, transactionID?: string }>();

  const cardID = React.useMemo(() => qsCardID ? parseInt(qsCardID, 10) : undefined, [qsCardID]);
    ...
    <Page>
      <Headbar.Container>
        ...
        <Headbar.Breadcrumbs
          ...
          transactionName={qsTransactionID ? `Transaction ID ${qsTransactionID}` : undefined}
        />
      </Headbar.Container>
