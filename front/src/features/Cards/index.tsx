...
const CardsPage = observer(() => {
  ...
  const { cardsFilters: filters, getCardsFilters } = useModel<FiltersModelT>("FiltersModel");
  const { loading, cards, totalCount, getCards } = useModel<CardsModelT>("CardsModel");
  ...
  const { view: statusFilterView, value: status } = useFilter({
    ...
    values: filters.status.map(s => ({ value: s, label: `${s[0].toUpperCase()}${s.slice(1, s.length)}` })),
  });
  ...
  return (
    <Page>
      ...
      <Sidebar>
        ...
        {
          cardIDFilterView || accountFilterView || currencyFilterView || statusFilterView
            ? (
              <React.Fragment>
                {cardIDFilterView}
                {accountFilterView}
