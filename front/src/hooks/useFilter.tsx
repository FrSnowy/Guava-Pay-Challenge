...
const useFilter = <T extends FilterValues>({ name, paramName, values }: T) => {
  const [current, setCurrent] = useSessionStoredState<string | undefined>(undefined, paramName);

  const label = React.useMemo(() => values.find(v => v.value === current)?.label, [values]);

  const onRadioClick = React.useCallback((v: string, l: string) => {
    if (current === v) {
  ...
  const filterVariantsView = React.useMemo(() => {
    return values.map(({ label, value }) => (
      <Radio
        ...
        onClick={() => onRadioClick(value, label)}
      />
    ))
  }, [values, current]);
