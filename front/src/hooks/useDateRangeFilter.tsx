...
const useDateRangeFilter = ({ paramName }: DateRangeFilterT) => {
  const [current, setCurrent] = useSessionStoredState<
    [string, string] | undefined
  >(undefined, paramName);

  const onCalendarChangeHandler = React.useCallback((v: unknown) => {
    ...
    const isoDates = datesRange.map((d) =>
      dateWithoutOffset(d).toISOString().replace(':000Z', '')
    ) as [string, string];
    setCurrent(isoDates);
  }, []);
  ...
  const renderValues = React.useMemo<[Date, Date] | null>(
    () =>
      current
        ? [
            new Date(
              Date.parse(current[0]) + new Date().getTimezoneOffset() * 60000
            ),
            new Date(
              Date.parse(current[1]) + new Date().getTimezoneOffset() * 60000
            ),
          ]
        : null,
    [current]
