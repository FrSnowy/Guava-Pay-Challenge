...
const Accordion: React.FC<AccordionProps> = ({ name, resetIcon, defaultVisibility, children }) => {
  const [isVisible, setIsVisible] = React.useState<boolean>(!!defaultVisibility);

  return (
    <SemanticAccordion className={styles.container}>
      <SemanticAccordion.Title active={isVisible} onClick={() => setIsVisible(!isVisible)}>
        <Icon name='dropdown' />
        {name}
