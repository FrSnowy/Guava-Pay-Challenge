...
const routeNames: Record<
  typeof AppRoutesConfig[number]['path'],
  (p: BreadcrumbsT) => string
> = {
  '/auth': () => 'Authorization',
  '/transactions': () => 'Institution transactions',
  '/transactions/:transactionID': ({ transactionName }) =>
    transactionName || 'Unknown transaction',
  '/transactions/:transactionID/:cardID': ({ cardName }) =>
...
export const Breadcrumbs: React.FC<BreadcrumbsT> = (props) => {
  ...
  const breadcrumbs = pathnameAsArray
    .map((_, i) => {
      const slicedPathname = pathnameAsArray
        .slice(0, pathnameAsArray.length - i)
        .join('')
        .replace(/\/+/g, '/');
      if (slicedPathname === '/') return null;

      const matchedRoute = AppRoutesConfig.find((r) =>
        matchPath(r.path || '', slicedPathname)
      );
      const name = matchedRoute
        ? routeNames[matchedRoute.path](props)
        : undefined;

      return {
        name,
        path: slicedPathname,
        current: slicedPathname === pathname,
      };
    })
    .filter((v) => !!v)
    .reverse();
        ...
        b ? (
          <React.Fragment key={b.path}>
            {b.current ? (
              ...
              <Link
                to={b.path}
                className={clsx(styles.link, { [styles.active]: !!b?.current })}
              >
                {b.name}
              </Link>
            )}
...
export const MainPageNavigation: React.FC<{ onPageChange?: () => void }> = ({
  onPageChange,
}) => {
